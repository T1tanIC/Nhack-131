...
public class KillAura extends Mod {
	...
	public KillAura() {
		super(Group.COMBAT, "Aura", "Attacks nearby targets automatically");
	}
            ...
            if (!targetEntity.hitByEntity(mc.player)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (mc.player.isSprinting() && flag)  {
                    	mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_KNOCKBACK, mc.player.getSoundCategory(), 1.0F, 1.0F);
                        ++i;
                        flag1 = true;
                    }
                    ...
                    f = f + f1;
                    boolean flag3 = false;
                    double d0 = mc.player.distanceWalkedModified - mc.player.prevDistanceWalkedModified;
                    ...
                    if (flag5) {
                        if (i > 0) {
                            if (targetEntity instanceof EntityLivingBase) {
                                ((EntityLivingBase)targetEntity).knockBack(mc.player, (float)i * 0.5F, MathHelper.sin(mc.player.rotationYaw * 0.017453292F), -MathHelper.cos(mc.player.rotationYaw * 0.017453292F));
                            } else  {
                                targetEntity.addVelocity(-MathHelper.sin(mc.player.rotationYaw * 0.017453292F) * (float)i * 0.5F, 0.1D, MathHelper.cos(mc.player.rotationYaw * 0.017453292F) * (float)i * 0.5F);
                            }
                        ...
                        if (flag3) {
                            ...
                            for (EntityLivingBase entitylivingbase : mc.player.world.getEntitiesWithinAABB(EntityLivingBase.class, targetEntity.getEntityBoundingBox().expand(1.0D, 0.25D, 1.0D))) {
                                if (entitylivingbase != mc.player && entitylivingbase != targetEntity && !mc.player.isOnSameTeam(entitylivingbase) && mc.player.getDistanceSq(entitylivingbase) < 9.0D) {
                                    entitylivingbase.knockBack(mc.player, 0.4F, MathHelper.sin(mc.player.rotationYaw * 0.017453292F), -MathHelper.cos(mc.player.rotationYaw * 0.017453292F));
                                    entitylivingbase.attackEntityFrom(DamageSource.causePlayerDamage(mc.player), f3);
                                }
                            }

                            mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_SWEEP, mc.player.getSoundCategory(), 1.0F, 1.0F);
                        ...
                        if (flag2) {
                        	mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, mc.player.getSoundCategory(), 1.0F, 1.0F);
                        	mc.player.onCriticalHit(targetEntity);
                        }

                        if (!flag2 && !flag3) {
                            if (flag) {
                            	mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_STRONG, mc.player.getSoundCategory(), 1.0F, 1.0F);
                            } else {
                            	mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_WEAK, mc.player.getSoundCategory(), 1.0F, 1.0F);
                            }
                        }
                    	...
                    	mc.player.world.playSound(null, mc.player.posX, mc.player.posY, mc.player.posZ, SoundEvents.ENTITY_PLAYER_ATTACK_NODAMAGE, mc.player.getSoundCategory(), 1.0F, 1.0F);
