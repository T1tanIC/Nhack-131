...
public class Gui extends GuiScreen {
	...
	public void drawScreen(int mouseX2, int mouseY2, float partialTicks) {		
		...
		if (GuiSettings.infoBox.booleanValue()) {
			...
			drawStringWithShadow(ChatFormatting.RED + "Made by: " + ChatFormatting.GREEN + "@Pryrobite", 2, 12, 0xFF000000);
			drawStringWithShadow(ChatFormatting.RED + "Discord: " + ChatFormatting.GREEN + Mod.DISCORD, 2, 22, 0xFF000000);
			drawStringWithShadow(ChatFormatting.RED + "Github: " + ChatFormatting.GREEN + "https://github.com/@Pryrobite", 2, 32, 0xFF000000);
			...
			if (g != null) {
				...
				if (description != null) {
					...
					for (int i = 0; i < description.length; i++) {
						int y = g.y + 6 + (i * 10);
						int width = mc.fontRenderer.getStringWidth(description[i]);
	...
	public void drawGuiNode(int mouseX, int mouseY, GuiNode node, int aboveNodes, int nodes) {
		...
		drawStringWithShadow(text, (((g.x2 / scale) - (g.x / scale)) / 2) + (g.x / scale) - ((mc.fontRenderer.getStringWidth(text)) / 2), (g.y + (g.y2 - g.y) / 3) / scale, g.guiNode.getTextColor());
